...
| Application UX/reporting | Partial | Application UI now includes product-area navigation with selected-menu workspaces instead of one long dashboard, optimized dense viewport usage across screens, compact topbar/sidebar/panel spacing, scroll-contained tables with sticky headers, guided work-area sections, command-center summary, collapsible/off-canvas menu, sticky app header, grouped settings tabs with independent settings loading, editable risk/scanner/analysis/broker/stage/backtest/AI/notification settings, improved basket/universe management, pipeline readiness, grouped operator-style pipeline controls, operations snapshot, data-source/cache visibility, historical execution report, report infographics, candidate/reason drill-down, scoped pipeline run controls, and CSV exports for key persisted views. Advanced interactive charts remain. |
| Local credentials | Done | `appsettings.Local.json` is ignored by git and supports Dhan token/client id plus notification credentials. API/UI settings can update broker provider, Dhan credentials, Telegram token/chat id, and email SMTP credentials using write-only secret replacement fields. |
...
| Local scanner universe | Done | Scanner instruments, baskets, and universes are now persisted in SQLite, with ignored config used only as a fallback/seed when the database is empty. Active instruments, reusable baskets, and executable universes are visible and editable through API/dashboard. Universes now preserve direct instruments separately from expanded basket stocks, so saving a universe no longer turns basket members into duplicated direct stocks. Predefined Dhan-resolved baskets exist for Nifty 50 Core, Bank Nifty Core, and Nifty IT Core, and `Predefined Core Universe` scans 21 unique stocks from those baskets. Instrument, basket, and universe editors include Dhan lookup/autosuggestion to populate symbol, exchange, security id, and ISIN. |
...
| 10 | Accuracy feedback database | Partial | Persisted backtest runs/trades provide an initial accuracy dataset with read APIs and dashboard summaries, including overall and direction-level calibration. Manual recommendation/outcome feedback ledger now exists with API write/read, dashboard visibility, and grouped feedback calibration by source/recommendation. Calibration reports by confidence/regime remain. | Build verified. |
| 11 | Historical replay/backtesting | Partial | EOD scanner replay CLI and API/UI manual run exist with configurable max holding days, daily-bar stop/target simulation, conservative same-day stop-before-target assumption, next-close fallback, win/loss/flat/no-exit counts, win rate, average return, persisted backtest reports, read API endpoints, and dashboard visibility. True intraday target/stop replay remains. | Build verified; Dhan smoke replay completed for 2026-09-08 to 2026-09-10. |
| 12 | Paper trading | Partial | Simulated paper-order ledger exists with API/dashboard visibility, manual run from latest live-validation/opening-range trade candidates, manual mark-to-market using intraday broker data, order close status, exit price, return percent, and realized P&L persistence. Long-running replay remains. | Build verified. |
...
| 14 | API/application UI | Partial | API exposes health, database-backed active scanner instruments/baskets/universes with safe update support, data-source/cache status, editable risk/scanner/analysis/broker/stage/backtest/AI/notification settings with write-only Dhan/Telegram/email secret replacement, latest EOD runs/candidates, latest pre-market runs/decisions, latest opening-range runs/decisions, latest live-validation runs/decisions, latest monitor runs/events, latest backtest runs/trades, backtest accuracy and direction calibration summaries, latest paper trading runs/orders, latest AI analysis runs/decisions, latest event log entries, latest broker status, latest pipeline readiness/prerequisite status with duplicate instrument warnings, latest notification attempts, Dhan instrument search, and independent manual notification-only pipeline run endpoints. Manual run endpoints can now scope EOD, pre-market, opening-range, live-validation, monitor, AI, paper trading, and backtesting to one database universe, all active instruments, one saved basket, or one configured stock. A decoupled React/Vite application UI exists and consumes only the API, including independent selected-menu workspaces, denser viewport-optimized layouts, independent stage buttons, one-click ordered workflow run, collapsible/off-canvas navigation, sticky app header, expanded grouped settings editor with independent settings loading, lookup-assisted scanner instrument/basket/universe editors, workflow timeline, operations snapshot, historical execution report, report infographics, candidate/reason drill-down, CSV exports, data-source/cache visibility, stage reason summaries, backtest report visibility, paper-order visibility, AI visibility, event-log visibility, active-instrument visibility, pipeline readiness bars, calibration bars, and scoped run selectors. Advanced interactive charts remain. | API endpoints, API build, dashboard build, browser Settings/Pipeline views, and universe-scoped pipeline status verified. |
